...
class ActivitiesSection extends StatelessWidget {
    ...
    return BlocListener<ActivityRegistrationBloc, ActivityRegistrationState>(
      listener: (context, state) {
        if (state is ActivityRegistrationSuccess) {
          ...
          // تحديث الملف الشخصي بعد نجاح العملية
          context.read<StudentBloc>().add(RefreshStudentProfileEvent());
        }
        if (state is ActivityRegistrationError) {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildActivityCard(BuildContext context, ActivitiesEntity activity) {
    ...
    // استخدام BlocSelector للحصول على حالة التحميل فقط
    return BlocSelector<
      ActivityRegistrationBloc,
      ActivityRegistrationState,
      bool
    >(
      selector: (state) => state is ActivityRegistrationLoading,
        ...
        return Container(
          ...
          child: Row(
            children: [
              ...
              // زر الحذف (يظهر فقط للحالات المعلقة)
              if (activity.status == "Pending")
                IconButton(
                  onPressed: isLoading
                      ? null
                      : () {
                          final activityId = activity.localActivityId;
                          print("🔍 [UI] محاولة حذف النشاط - ID: $activityId");

                          if (activityId != null) {
                            print(
                              "🗑️ [UI] إرسال حدث حذف للنشاط رقم: $activityId",
                            );
                            try {
                              context.read<ActivityRegistrationBloc>().add(
                                DeleteRegisterActivityEvent(
                              ...
                              print("✅ [UI] تم إرسال حدث الحذف بنجاح");
                            } catch (e) {
                              print("❌ [UI] خطأ في إرسال الحدث: $e");
                            }
                          } else {
                            print("❌ [UI] activityId هو null!");
                          }
                        },
                  icon: isLoading
                      ? SizedBox(
